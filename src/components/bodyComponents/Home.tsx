    ...
    return(
        <div className="flex flex-col gap-2 w-full justify-start pb-12">
            ...
            <div className="flex justify-between mt-2 mx-auto w-fit gap-4">
                <Card>
                    <h1 className="text-black dark:text-white font-bold break-words max-w-[32vw] text-[gold]">Day: {currencySymbol}{dayAmount.toString().length >= 6 ? `${((dayAmount/1000).toFixed(2))}K` : dayAmount}</h1>
                </Card>
                <Card>
                    <h1 className="text-black dark:text-white font-bold break-words max-w-[32vw] text-[gold]">Week: {currencySymbol}{weekAmount.toString().length >= 6 ? `${((weekAmount/1000).toFixed(2))}K` : weekAmount}</h1>
                </Card>
                <Card>
                    <h1 className="text-black dark:text-white font-bold break-words max-w-[32vw] text-[gold]">Month: {currencySymbol}{monthAmount.toString().length >= 6 ? `${((monthAmount/1000).toFixed(2))}K` : monthAmount}</h1>
                </Card>
            </div>
            <ExpenseList/>
        </div>
