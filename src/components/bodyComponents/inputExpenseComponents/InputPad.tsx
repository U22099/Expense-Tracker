  ...
  return(
    <main className="flex gap-2">
      <section className="flex flex-col gap-2">
        ...
        <div className="flex justify-center items-center text-3xl shadow-lg active:shadow-none rounded-lg py-3 px-5 bg-white dark:bg-black dark:text-white font-bold select-none" onClick={() => setValue((prevValue: number) => parseInt(`${prevValue}${0}`))}>
          <p>0</p>
        </div>
      </section>
...
const Keypad = ({num, key, setValue}: {num: number, key: number, setValue: (arg: number | ((prevValue: number) => number)) => void}) => {
  return(
    <div key={key} className="flex justify-center items-center text-3xl shadow-lg active:shadow-none rounded-lg py-3 px-5 bg-white dark:bg-black dark:text-white font-bold select-none" onClick={() => setValue((prevValue: number) => parseInt(`${prevValue}${num}`))}>
      <p>{num}</p>
    </div>
  )
}